...
// 문항 편집은 딜 편집 모달(TeamDealFormModal)로 일원화 — 여기서는 읽기 전용 요약만 보여준다.
function SurveyQuestionSummary({ questions }: { questions: AdminSurveyQuestion[] }) {
    ...
    <div className="rounded-[18px] bg-white border border-[#e0e0e0] mb-4 overflow-hidden">
      ...
      >
        <div className="text-left">
          <p className="text-[14px] font-semibold text-[#1d1d1f]">요청서 문항</p>
          <p className="text-[12px] text-[#6e6e73] mt-0.5">
            신청자에게 요청하는 이행 정보 · 현재 {questions.length}개 문항
          </p>
        </div>
        ...
        <div className="border-t border-[#e0e0e0] px-6 py-5 space-y-3">
          {questions.length === 0 ? (
            <p className="text-[13px] text-[#6e6e73]">
              아직 문항이 없습니다. 문항이 있어야 딜이 고객에게 오픈(모집중)될 수 있습니다.
            </p>
          ) : (
            <ol className="space-y-2">
              {questions.map((question, index) => (
                <li key={question.id} className="flex items-center gap-2 text-[13px] text-[#1d1d1f]">
                  <span className="text-[#a1a1a6] tabular-nums w-5 text-right">{index + 1}.</span>
                  <span className="flex-1">{question.label}</span>
                  <span className="rounded-[9999px] bg-[#f5f5f7] px-2 py-0.5 text-[11px] text-[#6e6e73]">
                    {QUESTION_TYPE_LABEL[question.question_type]}
                  </span>
                  {question.required && (
                    <span className="rounded-[9999px] bg-blue-50 px-2 py-0.5 text-[11px] text-blue-700">필수</span>
                  )}
                </li>
              ))}
            </ol>
          )}
          <p className="text-[12px] text-[#6e6e73] pt-1 border-t border-[#e0e0e0]">
            문항 추가·수정은{' '}
            <Link href="/admin/team-deals" className="text-[#0066cc] hover:underline">
              팀 구매 관리
            </Link>
            의 딜 편집에서 할 수 있습니다.
          </p>
...
// 목록 레벨 제출 현황: 상태 배지 + 전체 문항 대비 답변 수. 판정(survey_status)은 서버 계산 그대로.
function SurveySubmissionBadge({
  member,
  questions,
}: {
  ...
  if (member.survey_status === 'none') {
    return <span className="text-[11px] text-[#a1a1a6]">문항 없음</span>
  }
  const questionIds = new Set(questions.map(question => question.id))
  const answered = member.responses.filter(response => questionIds.has(response.question_id)).length
  return (
    ...
    <>
      <tr onClick={onToggle} className="hover:bg-[#f5f5f7] transition-colors cursor-pointer">
        ...
        <td className="px-4 py-3">
          <SurveySubmissionBadge member={member} questions={payload.questions} />
        </td>
        <td className="px-4 py-3">
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          <SurveyQuestionSummary questions={payload.questions} />
