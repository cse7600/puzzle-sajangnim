    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-ink shadow-sm hover:bg-primary-hover transition-colors"
        >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          {/* 포인트 안내 배너 */}
          <div className="mb-5 rounded-xl border border-primary-dark/20 bg-accent-bg/60 px-5 py-3 flex items-center gap-3">
            <Coins className="h-5 w-5 text-primary-dark shrink-0" />
          ...
          <div className="mb-5 flex flex-wrap gap-2">
            {QA_CATEGORIES.map(cat => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${activeCategory === cat ? 'bg-primary text-ink' : 'border border-gray-200 bg-white text-gray-600 hover:bg-gray-50'}`}
              >
                {cat}
              </button>
            ))}
          ...
          {qaLoading ? (
            ...
            <div className="rounded-xl border border-gray-100 bg-white p-12 text-center">
              ...
              <button
                onClick={() => router.push('/knowledge/ask')}
                className="mt-4 text-primary-dark text-sm font-medium hover:underline"
              >
                질문하기 +1,000P
              </button>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="mb-2.5 flex flex-wrap items-center gap-1.5">
                      ...
                      {q.is_adopted && (
                        <span className="inline-flex items-center gap-1 rounded-md bg-accent-bg px-2 py-0.5 text-xs font-medium text-primary-dark">
                          <CheckCircle2 className="h-3 w-3" />채택됨
                        </span>
                      )}
                    </div>
                    <h3 className="text-[15px] font-semibold leading-snug text-gray-900 group-hover:text-primary-dark transition-colors">
                      {q.title}
                    </h3>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-[12px] text-primary-dark">
        ...
        <div className="space-y-5">
          <div className="rounded-xl border border-primary-dark/20 bg-accent-bg/60 p-5">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary">
              <Coins className="h-5 w-5 text-ink" />
            </div>
            ...
            <button
              onClick={() => router.push('/knowledge/ask')}
              className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-ink hover:bg-primary-hover transition-colors"
            >
