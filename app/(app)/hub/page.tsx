...
const PLATFORM_INFO = {
  ...
  meta: { name: '메타(인스타)', color: '#1877F2', payback: '4.0%' },
...
export default function HubPage() {
  ...
  const [form, setForm] = useState({ platform: 'naver', account_id: '', account_name: '', monthly_spend: '' })
  ...
  const selectedPlatformInfo = PLATFORM_INFO[form.platform as keyof typeof PLATFORM_INFO]
  const estimatedModalPayback = form.monthly_spend
    ? Math.round(
        Number(form.monthly_spend.replace(/,/g, '')) *
        parseFloat(selectedPlatformInfo?.payback ?? '0') / 100
      )
    : 0
            ...
            <>
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-[13px] text-[#6e6e73] mt-0.5">네이버·메타·구글·카카오 광고계정을 연결하고 페이백 받기</p>
                </div>
                <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-[18px] w-full max-w-[480px] mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-[13px] text-[#6e6e73] mb-5">광고계정 정보를 입력하면 퍼즐팀이 확인 후 페이백을 시작합니다</p>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[13px] font-medium text-[#1d1d1f] mb-1.5">계정 이름</label>
                <input
                  type="text"
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="flex-1 rounded-[9999px] border border-[#e0e0e0] py-3 text-[15px] text-[#6e6e73] hover:bg-[#f5f5f7] transition-colors">
                취소
              </button>
              <button
