...
## 독립 검토 결과 (gap-detector)

설계 문서 대비 독립 검토를 돌렸다. **Match Rate 92%, FR-01~FR-10 전부 충족**.
검토가 지적한 사항 두 가지를 아래에 반영했다.

1. `GET /api/auth/consent`는 이전에 405를 반환하던 자리다. 지금은 인증된 조회 엔드포인트로 **동사 의미가 바뀌었다.**
   외부 소비자는 없고 `/settings` 한 곳만 호출하므로 안전하지만, 라이브 라우트의 동사 변경이라 기록해 둔다.
2. 설계 문서의 "콜백이 유일한 진입 경로라 우회가 없다"는 근거는 **부정확하다.** 아래 리스크 항목으로 옮겼다.

## 남은 리스크 / 갭

- **동의 인터스티셜은 우회 가능하다(설계상 수용).** 콜백이 `/auth/consent`로 보낸 시점에 유저는 이미
  완전한 세션을 들고 있다. 여기서 주소창에 `/hub`를 직접 입력하면 동의 없이 앱에 들어간다 —
  콜백 밖에서 `profile_data.consent`를 확인하는 지점이 없고, `middleware.ts:207`은 로그인 상태로
  `/login`에 오는 유저를 오히려 `/hub`로 보낸다. 설계 문서의 "콜백이 유일한 진입 경로라 우회 불가"는
  사실과 다르다(다음 사이클에서 이 문장을 근거로 재사용하지 말 것).
  이번 사이클에서 미들웨어 게이트를 추가하지 않은 것은 **의도적 지시**다 — `middleware.ts`가 이 저장소
  최고 리스크 파일이고 라이브 로그인 흐름의 회귀 비용이 우회 리스크보다 크다고 판단했다.
  실제로 막으려면 `handleOnboardingGate` 앞단에 consent 게이트를 하나 더 두는 방식이 되는데,
  그때는 5단 게이트 순서 전체를 다시 검증해야 한다.
